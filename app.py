...
with st.sidebar:
    ...
    st.caption(f"Perte de couplage source-fibre par défaut = {source_preset['coupler_db']:.1f} dB")

    st.subheader("Fibre optique")
    fiber_label = st.selectbox("Type de fibre", list(FIBER_CATALOG.keys()))
    fiber_catalog = FIBER_CATALOG[fiber_label]
    fiber_type = fiber_catalog["type"]
    ...
    with col_a:
        alpha = st.number_input("α (dB/km)", min_value=0.01, max_value=10.0,
                                value=float(fiber_catalog["alpha"]), step=0.05, format="%.3f")
    with col_b:
        Dch = st.number_input("Dch (ps/nm/km)", min_value=-200.0, max_value=200.0,
                              value=float(default_Dch), step=1.0)

    bandwidth_distance_MHz_km = fiber_catalog["bandwidth_distance_MHz_km"]
    ...
    with col_nc:
        Nc = st.number_input("Nb connecteurs", min_value=0, max_value=50, value=2, step=1)
    with col_lc:
    ...
    if splice_mode == "Manuel (Ns)":
        Ns = st.number_input("Nb épissures", min_value=0, max_value=500, value=11, step=1)
    ...
    loss_per_splice = st.number_input("Perte/épissure (dB)", min_value=0.0,
                                      max_value=2.0, value=0.3, step=0.01, format="%.3f")
...
with c3:
    ...
    st.metric("Bande passante BP", bp_str, delta=bp_delta, delta_color="off")
...
L_plot = np.linspace(
    0.1,
    max(200.0, L_km * 2, lmax.L_max_km * 1.3 if lmax.L_max_km != float("inf") else 200.0),
    600,
)
...
with tab0:
    ...
    fig_schema.add_annotation(x=1.3, y=5.15, text=f"<b>{source_type}</b>",
    ...
    fig_schema.add_shape(type="rect", x0=2.2, x1=7.8, y0=5.0 - core_h, y1=5.0 + core_h,
    ...
    fig_schema.add_annotation(x=5.0, y=4.25, text="Chaîne optique de transmission",
                              showarrow=False, font=dict(size=10, color="#555"))

    for cx, label in [(2.6, "C1"), (7.4, f"C{Nc}")]:
    ...
    if n_splice_shown > 0:
        fig_schema.add_annotation(x=splice_positions[0], y=6.2, text=f"Épissures (×{Ns})",
                                  showarrow=False, font=dict(size=9, color="#FF6600"))

    fig_schema.add_annotation(x=2.15, y=5.0, ax=2.85, ay=5.0,
                              xref="x", yref="y", axref="x", ayref="y",
                              showarrow=True, arrowhead=2, arrowsize=1.5,
                              arrowwidth=2, arrowcolor=fiber_color, text="")
    fig_schema.add_annotation(x=7.85, y=5.0, ax=7.15, ay=5.0,
                              xref="x", yref="y", axref="x", ayref="y",
                              showarrow=True, arrowhead=2, arrowsize=1.5,
                              arrowwidth=2, arrowcolor=fiber_color, text="")
    ...
    fig_schema.add_annotation(x=8.7, y=5.15, text=f"<b>{detector}</b>",
                              showarrow=False, font=dict(size=11, color="white"))
    fig_schema.add_annotation(x=8.7, y=4.5, text=f"Pr = {pb.Pr_dbm:.1f} dBm",
                              showarrow=False, font=dict(size=10, color="#DDAAFF"))
    fig_schema.add_annotation(x=8.7, y=3.85, text=f"Marge = {pb.margin_db:+.1f} dB",
                              showarrow=False, font=dict(size=10, color="#AAFFCC" if pb.is_feasible else "#FF8888"))
    ...
    fig_schema.add_annotation(x=1.3, y=2.9, text=f"Δλ = {delta_lambda:.1f} nm",
                              showarrow=False, font=dict(size=10, color="#555"))
    fig_schema.add_annotation(x=8.7, y=2.9, text=f"Débit = {bitrate_GHz:.3f} GHz",
                              showarrow=False, font=dict(size=10, color="#555"))
...
with tab2:
    ...
    finite_bw = bw_arr[bw_arr != np.inf]
    bw_cap = min(1e4, float(np.nanmax(finite_bw)) * 1.2) if finite_bw.size else 1e4
    bw_plot = np.clip(bw_arr, 0, bw_cap)
    fig2.add_trace(go.Scatter(
        x=L_plot, y=bw_plot,
